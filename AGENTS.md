...
`gadoo` is a CLI tool for managing the lifecycle of custom Odoo addons.
The primary workflow is a developer running `gadoo upgrade foo` or
`gadoo install foo` during active addon development.
...
— not like terse Unix tool output.
